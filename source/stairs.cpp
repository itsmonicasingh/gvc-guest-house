...
void renderStairwell(double rise, double run, double width, int steps) 
{
	...
	for (i = 0, riseDist = 0.0, runDist = 0.0; i < steps;i++, riseDist += rise, runDist += run) {
	...
	//draw stairwell's final riser
	glPushMatrix();
	glTranslated(0.0, riseDist, runDist);
	renderRectangle(0.0, 0.0, width, rise);
	glPopMatrix();
	//unset railing material
...
void makeStairs()
{
	...
	renderStairwell(0.1,0.1, 0.9, 9); //stairs for ground floor
	glPopMatrix();

	glPushMatrix();
	glTranslated(3.6,1.0,1.0);
	glScaled(1.0, 1.0, -1.0);
	renderStairwell(0.1,0.1, 0.9, 9); //stairs for first floor 
	glPopMatrix();
}
