...
void make_Railing()
{
	...
	int i;
	glColor3f(.8,.8,.8);  //color of railing
	for( i = 1; i <= 2; i++ ) {     //left side two railing 
		railing(x1,y1,z1,x2,y2,z2);
		x1+=1.0;
		x2+=1.0;
	}

	railing_three(x1,y1,z1,x2,y2,z2);   //third railing over curve
	x1++;
	x2++;

	railing_four(x1,y1,z1,x2,y2,z2);    //fourth railing over curve
	x1++;
	x2++;

	railing_five(x1,y1,z1,x2,y2,z2);    //fifth railing over curve
	x1++;
	x2++;

	railing_six(x1,y1,z1,x2,y2,z2);     //sixth railing over curve
	x1++;
	x2++;

	for( i = 7; i <= 10; i++ ) {       // rest railing on 
		railing(x1,y1,z1,x2,y2,z2);
		x1+=1.0;
		x2+=1.0;
	}
	railing_side(3.0,1.5,3.0,3.0,1.0,3.0);  //side railing of gorund floor

	x1 = -7.0f; y1 = 2.5f; z1 = 1.5f;
	x2 = -7.0f; y2 = 2.0f; z2 = 1.5f;

	for( i = 1; i <= 10; i++ ) {         
		railing(x1,y1,z1,x2,y2,z2);    //railing of first floor 
		x1+=1.0;
		x2+=1.0;
	}
	railing_side(3.0,2.5,3.0,3.0,2.0,3.0);  //side railing of first floor 
}
