...
GLvoid InitGL(GLsizei Width, GLsizei Height)	// We call this right after our OpenGL window is created.
{
	...
	glEnable(GL_DEPTH_TEST);                    // enables depth testing.remove transparency
...
/* function to draw model for guest house */
GLvoid DrawGLScene()
{
	...
	glColor3f(0.7,0.7,0.7);  //color for pillars 
	...
	for(i = -7.0f;i < 3.0f; i += 2.0f){
		render_Walls_Lower(i,0.0f,1.0f,-3.0f); //walls of ground floor
		render_Walls_Upper(i,1.0f,0.2f,-3.0f); //walls of first floor
	}
	...
	for(i = -5.05f;i < 3.0f;i += 2.0f){
		...
		renderBed(); //bed for ground floor 
		glPopMatrix();
		
		glPushMatrix();
		glTranslated(i, 1.01, -0.5);
		glRotated(90, 0.0, 1.0, 0.0);
		renderBed();   //bed for first floor
		glPopMatrix();
	}
...
{  
	/* load coordinest of house from file */
	SetupWorld(); 
	glutInit(&argc, argv);   
	glutInitDisplayMode( GLUT_DOUBLE | GLUT_DEPTH | GLUT_RGBA );  
	
	//Antialiashing calculation started
